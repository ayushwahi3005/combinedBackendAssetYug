...
public class CompanyCustomerAPI {
	...
	public CompanyCustomerDTO getCompanyCustomerByLocalId(@PathVariable String id,@PathVariable String companyId){
		System.out.println(id);
...
//	}
	...
	public void addExtraFieldName(@RequestBody CompanyCustomerExtraFieldNameDTO extraFieldNameDTO) throws Exception {
...
//		System.out.println("----------my company------------->"+companyId);
	...
	public ResponseEntity<CompanyCustomerMandatoryFields> getMandatoryFields(@PathVariable String name,@PathVariable String companyId){
		System.out.println("============================>"+name+companyId);
		CompanyCustomerMandatoryFields mandatoryFields=companyCustomerService.getMandatoryFields(name,companyId);
		return ResponseEntity.ok(mandatoryFields);
	}
	@GetMapping("/getShowFields/{name}/{companyId}")
	public ResponseEntity<CompanyCustomerShowFields> getShowFields(@PathVariable String name,@PathVariable String companyId){
	...
	public ResponseEntity<List<CompanyCustomerMandatoryFields>> getAllMandatoryFields(@PathVariable String companyId){
		List<CompanyCustomerMandatoryFields> mandatoryFieldsList=companyCustomerService.getAllMandatoryFields(companyId);
		return ResponseEntity.ok(mandatoryFieldsList);
	}
	@GetMapping("/getAllShowFields/{companyId}")
	public ResponseEntity<List<CompanyCustomerShowFields>> getAllShowFields(@PathVariable String companyId){
		List<CompanyCustomerShowFields> showFieldsList=companyCustomerService.getAllShowFields(companyId);
	...
	public void addNewFields(@RequestBody CompanyCustomerExtraFieldsDTO extraFieldsDTO) throws Exception {
					...
					if(columnMap.get(field)!=null) {
						switch (columnMap.get(field).toLowerCase()) {
							...
							case "status":
								...
								if ((row[j].toLowerCase().equals("active")) || (row[j].toLowerCase().equals("inactive"))) {
...
//		                //System.out.println("-------|||||-------> "+columnMap.get(field).toLowerCase());
						...
						List<CompanyCustomerExtraFieldName> listExtraFieldName= extraFieldNameRepository.findByCompanyId(companyId);
						if(columnMap.get(field)!=null) {
							for (int i = 0; i < listExtraFieldName.size(); i++) {
								if ((columnMap.get(field) != null) && columnMap.get(field).toLowerCase().equals(listExtraFieldName.get(i).getName().toLowerCase())) {
									...
									if (errorFlag == 0) {
										...
										CompanyCustomer myCustomer = modelMapper.map(mynewCustomer, CompanyCustomer.class);
...
//		System.out.println(importHistoryDTO);
	...
	public void importUpdation(@RequestParam("file") MultipartFile file,@RequestParam("columnMappings") String columnMappings,@PathVariable String companyId,@PathVariable String email) throws CsvValidationException, JsonParseException, IOException, MessagingException, ImportFileRowException {
...
//	                    }
								companyCustomer = companyCustomerRepository.findByCompanyCustomerIdAndCompanyId(Integer.parseInt(companyCustomerIdValue), companyId);
...
//		               System.out.println("--------id------>"+id);
							for (int i = 0; i < listExtraFieldName.size(); i++) {
								if (columnMap.get(field).toLowerCase().equals(listExtraFieldName.get(i).getName().toLowerCase())) {
									CompanyCustomerExtraFields extraFieldsDTO = new CompanyCustomerExtraFields();
									CompanyCustomerExtraFields extraFieldsOptional = extraFieldsRepository.findByNameAndCompanyCustomerId(listExtraFieldName.get(i).getName(), id);
									System.out.println("-----------------working ---->" + listExtraFieldName.get(i).getType());
...
//		System.out.println(importHistoryDTO);
	...
	public ResponseEntity<List<String>> statelist(){
		List<String> stateList=List.of("Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Carolina", "North Dakota", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Rhode Island", "South Carolina", "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "West Virginia", "Wisconsin", "Wyoming"
		);
	...
	public CompanyCustomerCategory getCategoryById(@PathVariable String companyId,@PathVariable String id){
