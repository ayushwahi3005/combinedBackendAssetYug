...
public class CompanyCustomerServiceImpl implements CompanyCustomerService {
	...
	public CompanyCustomerDTO addCustomer(CompanyCustomerDTO myCustomerDTO) {
		...
		if(companyCustomer.getCompanyCustomerId()==null) {
			Optional<CompanyCustomerIdTable> optionalIdTable=idTableRepository.findByCompanyId(myCustomerDTO.getCompanyId());
...
//		WorkOrderWithExtraFieldsDTO workOrderWithExtraFieldsDTO = modelMapper.map(workOrder, WorkOrderWithExtraFieldsDTO.class);
		...
		workOrderList.stream().forEach((order)->{
			List<CompanyCustomerExtraFields> extraFieldsList=extraFieldsRepository.findByCompanyCustomerId(order.getId());
...
//	            System.out.print(m);
	...
	public void addCompanyCustomerExtraField(CompanyCustomerExtraFieldNameDTO extraFieldNameDTO) throws ExtraFieldAlreadyPresentException {
		// TODO Auto-generated method stub
		CompanyCustomerExtraFieldName extraFieldNameNew=extraFieldNameRepository.findByNameAndCompanyId(extraFieldNameDTO.getName().toLowerCase(),extraFieldNameDTO.getCompanyId());
		...
		CompanyCustomerExtraFieldName extraFieldName=modelMapper.map(extraFieldNameDTO, CompanyCustomerExtraFieldName.class);
	...
	public List<CompanyCustomerExtraFieldNameDTO> getCompanyCustomerExtraField(String companyId) {
		...
		extraFieldNameList.stream().forEach((x)->{
			CompanyCustomerExtraFieldNameDTO extraFieldNameDTO=modelMapper.map(x, CompanyCustomerExtraFieldNameDTO.class);
	...
	public void deleteCompanyCustomerExtraField(String id) {
		...
		List<CompanyCustomerExtraFields> extraFieldsList=extraFieldsRepository.findByName(extraFieldName.getName().toLowerCase());
	...
	public void updateMandatoryFields(CompanyCustomerMandatoryFields mandatoryFields) {
		// TODO Auto-generated method stub
		Optional<CompanyCustomerMandatoryFields> mandatoryFieldsOptional=mandatoryFieldsRepository.findByNameAndCompanyId(mandatoryFields.getName(),mandatoryFields.getCompanyId());
	...
	public void updateShowFields(CompanyCustomerShowFields showFields) {
		// TODO Auto-generated method stub
		Optional<CompanyCustomerShowFields> showFieldsOptional=showFieldsRepository.findByNameAndCompanyId(showFields.getName(),showFields.getCompanyId());
	...
	public CompanyCustomerMandatoryFields getMandatoryFields(String name, String companyId) {
		// TODO Auto-generated method stub
		Optional<CompanyCustomerMandatoryFields> mandatoryFieldsOptional=mandatoryFieldsRepository.findByNameAndCompanyId(name,companyId);
		if(mandatoryFieldsOptional.isPresent()) {
	...
	public CompanyCustomerShowFields getShowFields(String name, String companyId) {
		...
		System.out.println("Servie====>"+name+" "+companyId);
				Optional<CompanyCustomerShowFields> showFieldsOptional=showFieldsRepository.findByNameAndCompanyId(name,companyId);
...
//		List<CompanyCustomerExtraFields> extraFieldsList=extraFieldsRepository.findByName(extraFieldsDTO.getName().toLowerCase());
...
List<CompanyCustomerExtraFields> extraFieldsList=extraFieldsRepository.findByCompanyCustomerId(id);
	...
	public List<CompanyCustomerFileDTO> getCompanyCustomerFile(String companyCustomerId) {
		...
		List<CompanyCustomerFile> companyCustomerList=companyCustomerFileRepository.findByCompanyCustomerId(companyCustomerId);
	...
	public CompanyCustomerDTO getCompanyCustomerByLocalId(Integer id,String companyId) {
		// TODO Auto-generated method stub
		CompanyCustomer companyCustomerOptional=companyCustomerRepository.findByCompanyCustomerIdAndCompanyId(id,companyId);
	...
	public PaginatedResultDTO<String> sortCustomers(String companyId, String field, Integer pageNumber, Integer pageSize) {
		...
		CompanyCustomerExtraFieldName extraFieldName=extraFieldNameRepository.findByNameAndCompanyId(field, companyId);
...
//	                //System.out.println("Key: " + key + ", Value: " + value);
			...
			PaginatedResultDTO<String> assetsWithAllFields=getAllCustomerDetails(mapping.get("companyId"));
			System.out.println("total1->"+assetsWithAllFields.getData().size()+" "+assetsWithAllFields.getTotalRecords());
				...
				try {
					...
					for (Map.Entry<?, ?> entry : filterMap.entrySet()) {
						...
						if (value != null) {
							...
							if (!keyString.equals("companyId") && myValue != null && !value.toString().isEmpty()) {
...
//	            Sorting if it is enabled
			...
			if(sortField!=null&&(sortField.trim().equals("")==false)) {
				...
				ObjectMapper objectMapper = new ObjectMapper();
				Comparator<String> customComparator = Comparator.comparing(data->{
                    System.out.println("inside comparator-"+ (String) data);
					Map<String, String> myMap=new HashMap<>();
					try {
						myMap = objectMapper.readValue((String) data, new TypeReference<Map<String, String>>() {});
					} catch (JsonMappingException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					} catch (JsonProcessingException e) {
...
//			System.out.println("total3->"+filteredAssetsWithAllFields.size()+" "+searchData.length()+" "+searchData.charAt(1));

			if(!searchData.isEmpty()&&searchData!=""){
				System.out.println("---------->"+searchData);
				filteredAssetsWithAllFields=filteredAssetsWithAllFields.stream().filter((data)->
						data.toLowerCase().contains(searchData.toLowerCase())
				).collect(Collectors.toList());
			}
	...
	public void addCategory(CategoryDTO categoryDTO) throws CategoryException {
		...
		Optional<CompanyCustomerCategory> OptionalCompanyCustomerCategory = companyCustomerCategoryRepository.findByName(categoryDTO.getName());
	...
	public List<CompanyCustomerCategory> getActiveCategoryList(String companyId) {
		List<CompanyCustomerCategory> categoryList= companyCustomerCategoryRepository.findByCompanyIdAndStatus(companyId,"active");
		return categoryList;
	}
